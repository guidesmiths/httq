...
describe('requestToSchemaUrl', function() {
    ...
    it('should construct a routing key from the template vars', function(done) {
        ...
        requestToSchemaUrl(config, {}, function(err, _middleware) {
            assert.ifError(err)
            middleware = _middleware
            request({url: 'http://localhost:3000'}, function(err, response, content) {
                assert.ifError(err)
                assert.equal(response.statusCode, 204)
                assert.equal(httq.message.schema, 'http://s3.amazonws.com/schemas/1/2/3/GET.json')
                done()
            })
        })
    })
    ...
    it('should map the method', function(done) {
        ...
        requestToSchemaUrl(config, {}, function(err, _middleware) {
            ...
            request({url: 'http://localhost:3000'}, function(err, response, content) {
                assert.ifError(err)
                assert.equal(response.statusCode, 204)
                assert.equal(httq.message.schema, 'http://s3.amazonws.com/schemas/1/2/3/requested.json')
                done()
            })
        })
    })
