...
function sourcedJsonValidator(fetchJsonSchema, config, ctx, next) {
    ...
    next(null, function(req, res, next) {
        ...
        function ensureSchemas(next) {
            ...
            if (!req.httq.message.schema) return next(new Error(format('No schema defined for %s', req.url)))

            var schemaUrl = req.httq.message.schema
        ...
        function validate(schemaUrl, next) {
            ...
            var result = validator.validateMultiple(req.httq.message.content.body, schema);
